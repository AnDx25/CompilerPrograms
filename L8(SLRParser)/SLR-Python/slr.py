...
def main():
	...
	print "inverse production==",inverse_production
	...
	table2 = [["Stack", "Input Stack", "Action"]]### string check table
	...
	while condition:
		...
		if action == "S":# shift action in table
			stack.append(input_stack[0])
			stack.append(state)
			
			del input_stack[0]
			
		elif action == "R":# reduce action in table
			location = int(state)
	...
	print "\n\nAction table\n" 
	print xy  # actual parsing action happend here
